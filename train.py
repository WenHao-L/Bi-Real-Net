...
from src.tools.criterion import get_criterion, NetWithLoss, CrossEntropySmooth
...
def main():
    ...
    # set lr_lambda
    args.lr_lambda = lambda step : (1.0-step/args.epochs)
    optimizer = get_optimizer(args, net, batch_num)
    # save a yaml file to read to record parameters

    net_with_loss = get_train_one_step(args, net_with_loss, optimizer)

    # eval_criterion has no label_smoothing
    eval_criterion = CrossEntropySmooth(sparse=True, reduction="mean", num_classes=args.num_classes)
    eval_network = nn.WithEvalCell(net, eval_criterion, args.amp_level in ["O2", "O3", "auto"])
    eval_indexes = [0, 1, 2]
